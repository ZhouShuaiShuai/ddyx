...
public class JWTToken {
    ...
    public static String buildAdminJwt(AdminUser adminUser) {
        ...
        String jwt = Jwts
                ...
                .setExpiration(new Date(time))
                //该方法是在JWT中加入值为vaule的key字段
                .claim("key", JSON.toJSONString(adminUser))
                .compact();
        userMap.put(adminUser.getId(), jwt);
        System.out.println("userMap : "+userMap);
        return jwt;
    }
    ...
    public static AdminUser getAdminUserFromJwt(String jwt) {
        try {
            Claims claims = Jwts.parser()
                    //SECRET_KEY是加密算法对应的密钥，jjwt可以自动判断机密算法
                    .setSigningKey(SECRET_KEY)
